...
function spanGridRows(content) {
  ...
  // Remove any previously set `--body-grid-row` properties otherwise the
  // `rowsToSpan` calculation fails miserably.
  ...
  for (const media in customMediaMinMax) {
    ...
    if (myMedia.matches) {
      const totalLinesOfText = Math.floor(contentHeight / lineHeight);
      const totalGutters = gridRowGapUnit * Math.floor(totalLinesOfText / linesWithGutter);
      ...
      if (totalGutters >= gridRowGapUnit) {
        ...
        const totalHeight = contentHeight - totalGutters;
        const gapsToSpan = Math.floor(totalHeight / computedRowHeight);
